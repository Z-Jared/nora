...
- 预计: 1 hour
- Worker: Claude A
- 依赖: TASK-176A、TASK-176B 完成并集成。
- 目标: 在 Pet Room 中添加 deterministic room-load greeting，让 Nora-01 在房间加载时基于 bounded pet state 和 local time bucket 显示一句简短、安全、text-only 的 presence greeting，增强“回到房间它在”的感觉。
- 非目标: 不实现真实 TTS、音频播放、录音、麦克风、provider adapter、PWA/service worker、桌面浮窗、通知、3D/VRM、billing、marketplace、云同步、LLM-generated greeting、relationship memory 写入或 activity 写入。
- 安全边界: DOM/text-only；greeting 必须由现有 bounded state 与 coarse local time bucket 派生；不得调用 provider/network；不得读取麦克风、摄像头、屏幕或位置；不得修改 pet state、food、activity、relationship memory、voice preview state 或 consent state；动态文本必须使用 DOM text APIs；不得新增诱导付费、voice cloning、always/background listening、surveillance、marketplace、notification、PWA 或 3D/VRM 文案。
- 持久证据: Pet Room 暴露稳定 greeting markers，例如 `pet-room-greeting`, `pet-room-greeting-text`, `pet-room-greeting-meta`；mapping helper 从 bounded state/time bucket 生成 label/meta 且无 fetch/mutation side effect；malformed state/time fallback bounded。
...
- 预计: 1 hour
- Worker: Claude B
- 依赖: TASK-176A、TASK-176B 完成并集成；与 TASK-177A 并行，但不得改实现文件。
- 目标: 为 deterministic room-load greeting 增加 eval/smoke 覆盖，锁住 greeting DOM markers、state/time-bucket mapping、read-only behavior、malformed fallback，以及 no voice/native/PWA/notification/3D/billing/marketplace scope drift。
- 非目标: 不实现 UI/CSS/JS greeting mapping；不修改 `mini_agent/static/index.html`、`mini_agent/tts.py`、`mini_agent/pets.py` 或 HTTP handler；不增加真实 TTS、录音、PWA、service worker、通知、桌面浮窗、支付、marketplace 或 3D/VRM。
- 安全边界: eval 必须阻断 voice cloning、recording by default、always/background listening、microphone/camera/screen/location access、hidden cost、purchase pressure、marketplace drift、3D/VRM implementation drift、PWA/service-worker/notification drift；不得只检查文件存在，必须锁住 mapping behavior、read-only behavior 或 copy contract。
- 持久证据: 新增 eval case 名称包含 `room_greeting` 或 `pet_room_greeting`; TASK-177A 合并后 eval 必须 active/pass，不能长期 skip。
