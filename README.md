...
GET  /tools               列出可用工具
GET  /task                 获取当前任务（无任务返回 null）
POST /task/start          创建任务 {"goal": "...", "steps": "步骤一\n步骤二"}
...
`/status` 返回字段：`auth_required`（是否需要 token）、`provider`/`model`（当前 LLM 配置，可为空）、`llm_configured`（provider、model、API key 是否齐全）、`config_warnings`（缺失配置项提示，齐全时为空数组）、`workspace`（工作目录）、`features`（sessions/tasks/memory/websocket 是否可用）、`runtime`（Python 版本、平台信息）。不泄露 API key、token 或环境变量。
