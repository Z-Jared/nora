...
`/status` 返回字段：`auth_required`（是否需要 token）、`provider`/`model`（当前 LLM 配置，可为空）、`llm_configured`（provider、model、API key 是否齐全）、`workspace`（工作目录）、`features`（sessions/tasks/memory/websocket 是否可用）、`runtime`（Python 版本、平台信息）。不泄露 API key、token 或环境变量。
