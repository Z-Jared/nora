...
def main() -> int:
    cases = [
        ...
        EvalCase("workspace_file_inspection_valid_list_read_preview", eval_workspace_file_inspection_valid_list_read_preview),
        EvalCase("workspace_file_inspection_path_escape_rejected", eval_workspace_file_inspection_path_escape_rejected),
        EvalCase("workspace_file_inspection_missing_lease_worker_mismatch", eval_workspace_file_inspection_missing_lease_worker_mismatch),
        EvalCase("workspace_file_inspection_offline_idle_rejected", eval_workspace_file_inspection_offline_idle_rejected),
        EvalCase("workspace_file_inspection_safety_no_leak", eval_workspace_file_inspection_safety_no_leak),
        ...
        # TASK-082: Finalize merge evals
        EvalCase("finalize_merge_successful_finalization", eval_finalize_merge_successful_finalization),
        EvalCase("finalize_merge_guard_rails", eval_finalize_merge_guard_rails),
        ...
        # TASK-096: Scheduler retry planning evals
        EvalCase("retry_planner_available", eval_retry_planner_available),
        EvalCase("retry_planner_exhausted", eval_retry_planner_exhausted),
        EvalCase("retry_planner_blocked_active_worker", eval_retry_planner_blocked_active_worker),
        EvalCase("retry_explain_available", eval_retry_explain_available),
        EvalCase("retry_explain_exhausted", eval_retry_explain_exhausted),
        EvalCase("retry_explain_blocked_active_worker", eval_retry_explain_blocked_active_worker),
        EvalCase("retry_explain_missing_capacity", eval_retry_explain_missing_capacity),
        EvalCase("retry_priority_vs_closeout", eval_retry_priority_vs_closeout),
        EvalCase("retry_priority_vs_dispatch", eval_retry_priority_vs_dispatch),
        EvalCase("retry_filter_no_leak", eval_retry_filter_no_leak),
        ...
        EvalCase("policy_hook_summary_counts", eval_policy_hook_summary_counts),
        EvalCase("policy_hook_summary_filter_hook", eval_policy_hook_summary_filter_hook),
        EvalCase("policy_hook_summary_filter_decision", eval_policy_hook_summary_filter_decision),
        EvalCase("policy_hook_summary_filter_category_risk", eval_policy_hook_summary_filter_category_risk),
        EvalCase("policy_hook_summary_filter_linkage", eval_policy_hook_summary_filter_linkage),
        EvalCase("policy_hook_summary_limit", eval_policy_hook_summary_limit),
        EvalCase("policy_hook_summary_invalid_filters", eval_policy_hook_summary_invalid_filters),
        EvalCase("policy_hook_summary_unsafe_linkage", eval_policy_hook_summary_unsafe_linkage),
...
# --- Workspace file inspection eval sentinels ---

_FILE_INSPECT_SENTINEL_GOAL = "NORA_EVAL_FILEINSPECT_GOAL_SENTINEL_x7y8z9"
_FILE_INSPECT_SENTINEL_SECRET = "NORA_EVAL_FILEINSPECT_SECRET_sk-fileinspect-a1b2c3d4"
_FILE_INSPECT_SENTINEL_STEP = "NORA_EVAL_FILEINSPECT_STEP_SENTINEL_e5f6g7h8"
_FILE_INSPECT_SENTINEL_FILE_CONTENT = "NORA_EVAL_FILEINSPECT_CONTENT_SECRET_i9j0k1l2"


def _setup_file_inspect_worker_with_files(registry, worker_id="w_fi"):
    """Helper: register worker, create task with sentinels, assign, set running, prepare workspace lease, create test files.
    Returns (task_id, workspace_path)."""
    ws = registry.durable_worker_store
    ts = registry.durable_task_store
    ws.register_worker(worker_id, role="coder")
    task = ts.create_task(goal=_FILE_INSPECT_SENTINEL_GOAL + " " + _FILE_INSPECT_SENTINEL_SECRET, steps=[{"text": _FILE_INSPECT_SENTINEL_STEP}])
    tid = task.task_id
    ts.assign_worker(tid, worker_id)
    ws.update_status(worker_id, "assigned", current_task_id=tid)
    ts.update_status(tid, "running")
    ...
    (ws_path / "data.txt").write_text(_FILE_INSPECT_SENTINEL_FILE_CONTENT, encoding="utf-8")
    return tid, str(ws_path)


def eval_workspace_file_inspection_valid_list_read_preview():
    """Valid scoped file inspection: list returns bounded relative paths, read reads inside lease, preview does not mutate."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            tid, ws_path = _setup_file_inspect_worker_with_files(registry)
            ws_root = Path(ws_path)

            # --- list_worker_workspace_files ---
            list_result = json.loads(registry.call(
                "list_worker_workspace_files", worker_id="w_fi", task_id=tid,
            ))
            assert "files" in list_result, f"list missing files key: {list_result}"
            assert "count" in list_result, f"list missing count key: {list_result}"
            assert list_result["count"] >= 2, f"expected >=2 files, got {list_result['count']}"
            # All paths are relative (no leading /)
            for f in list_result["files"]:
                assert not f.startswith("/"), f"list returned absolute path: {f}"
            assert "src/main.py" in list_result["files"], f"src/main.py not found: {list_result['files']}"
            assert "README.md" in list_result["files"], f"README.md not found: {list_result['files']}"
            # Bounded: lease info present
            assert "workspace_path" in list_result
            assert "lease_id" in list_result

            # --- read_worker_workspace_file ---
            read_result = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="src/main.py",
            ))
            assert "content" in read_result, f"read missing content: {read_result}"
            assert "print('hello')" in read_result["content"]
            assert read_result["path"] == "src/main.py"
            assert "size" in read_result
            assert "workspace_path" in read_result
            assert "lease_id" in read_result

            # Read with absolute path inside workspace
            abs_read = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid,
                path=str(ws_root / "README.md"),
            ))
            assert "content" in abs_read, f"absolute read failed: {abs_read}"
            assert "# Test Project" in abs_read["content"]

            # --- preview_worker_workspace_write ---
            # New file preview
            preview_new = json.loads(registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                path="new_file.py", content="print('new')\n",
            ))
            assert "preview" in preview_new, f"preview missing preview: {preview_new}"
            assert preview_new["will_create"] is True, f"new file should will_create=True"
            assert "print('new')" in preview_new["preview"]
            assert "workspace_path" in preview_new
            assert "lease_id" in preview_new

            # Existing file preview (diff)
            preview_exist = json.loads(registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                path="README.md", content="# Modified Project\n",
            ))
            assert "preview" in preview_exist
            assert preview_exist["will_create"] is False
            assert "-# Test Project" in preview_exist["preview"]
            assert "+# Modified Project" in preview_exist["preview"]

            # Preview does NOT mutate the file on disk
            assert (ws_root / "README.md").read_text(encoding="utf-8") == "# Test Project\n"
            assert not (ws_root / "new_file.py").exists(), "preview should not create new_file.py"

            # Preview does NOT mutate task/worker state
            task_after = json.loads(registry.call("get_durable_task", task_id=tid))
            assert task_after["status"] == "running", f"task status mutated: {task_after['status']}"
        finally:
            db.close()


def eval_workspace_file_inspection_path_escape_rejected():
    """Relative traversal and absolute path escape are rejected by all three tools."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            # --- Relative traversal ---
            # list has no path arg, skip it for traversal tests
            r_trav_read = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="../../etc/passwd",
            ))
            assert "error" in r_trav_read, f"read traversal should be rejected: {r_trav_read}"

            r_trav_prev = json.loads(registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                path="../../etc/evil", content="bad",
            ))
            assert "error" in r_trav_prev, f"preview traversal should be rejected: {r_trav_prev}"

            # --- Absolute path escape ---
            r_abs_read = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="/etc/passwd",
            ))
            assert "error" in r_abs_read, f"read absolute escape should be rejected: {r_abs_read}"

            r_abs_prev = json.loads(registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                path="/etc/evil", content="bad",
            ))
            assert "error" in r_abs_prev, f"preview absolute escape should be rejected: {r_abs_prev}"

            # --- Deep traversal ---
            deep_path = str(ws_root / "subdir" / ".." / ".." / ".." / "etc" / "shadow")
            r_deep = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path=deep_path,
            ))
            assert "error" in r_deep, f"deep traversal should be rejected: {r_deep}"

            # --- Empty path ---
            r_empty = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="",
            ))
            assert "error" in r_empty, f"empty path should error: {r_empty}"

            r_empty_prev = json.loads(registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid, path="", content="x",
            ))
            assert "error" in r_empty_prev, f"empty path preview should error: {r_empty_prev}"
        finally:
            db.close()


def eval_workspace_file_inspection_missing_lease_worker_mismatch():
    """No lease, unknown worker, task mismatch → JSON errors for all three tools."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            # Unknown worker
            for tool, extra_kw in [
                ("list_worker_workspace_files", {}),
                ("read_worker_workspace_file", {"path": "f.txt"}),
                ("preview_worker_workspace_write", {"path": "f.txt", "content": "x"}),
            ]:
                r = json.loads(registry.call(tool, worker_id="w_ghost", task_id="dtask_x", **extra_kw))
            ...
            for tool, extra_kw in [
                ...
                r = json.loads(registry.call(tool, worker_id="w_nolease", task_id=task_nl.task_id, **extra_kw))
            ...
            for tool, extra_kw in [
                ...
                r = json.loads(registry.call(tool, worker_id="w_mismatch", task_id=task_b.task_id, **extra_kw))
                assert "error" in r, f"task mismatch {tool} should error: {r}"
        finally:
            db.close()


def eval_workspace_file_inspection_offline_idle_rejected():
    """Offline and idle workers with stale current_task_id and lease are rejected by all three tools."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            # Offline worker
            tid_off, ws_path_off = _setup_file_inspect_worker_with_files(registry, worker_id="w_offline")
            ws.update_status("w_offline", "offline", current_task_id=tid_off)

            for tool, extra_kw in [
                ("list_worker_workspace_files", {}),
                ("read_worker_workspace_file", {"path": "README.md"}),
                ("preview_worker_workspace_write", {"path": "f.txt", "content": "x"}),
            ]:
                r = json.loads(registry.call(tool, worker_id="w_offline", task_id=tid_off, **extra_kw))
                assert "error" in r, f"offline {tool} should error: {r}"

            # Idle worker
            tid_idle, ws_path_idle = _setup_file_inspect_worker_with_files(registry, worker_id="w_idle")
            ...
            for tool, extra_kw in [
                ...
                r = json.loads(registry.call(tool, worker_id="w_idle", task_id=tid_idle, **extra_kw))
                assert "error" in r, f"idle {tool} should error: {r}"
        finally:
            db.close()


def eval_workspace_file_inspection_safety_no_leak():
    """Outputs do not leak raw task goals, steps, secrets, or file content sentinels."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            # list output should not leak goal/step
            list_r = registry.call("list_worker_workspace_files", worker_id="w_fi", task_id=tid)
            assert _FILE_INSPECT_SENTINEL_GOAL not in list_r, f"goal leaked in list: {list_r}"
            assert _FILE_INSPECT_SENTINEL_STEP not in list_r, f"step leaked in list: {list_r}"
            assert _FILE_INSPECT_SENTINEL_SECRET not in list_r, f"secret leaked in list: {list_r}"

            # read output: the file data.txt contains the sentinel content, that's expected in "content" field
            # but goal/step/secret should NOT leak
            read_r = registry.call("read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="data.txt")
            assert _FILE_INSPECT_SENTINEL_GOAL not in read_r, f"goal leaked in read: {read_r}"
            assert _FILE_INSPECT_SENTINEL_STEP not in read_r, f"step leaked in read: {read_r}"
            assert _FILE_INSPECT_SENTINEL_SECRET not in read_r, f"secret leaked in read: {read_r}"

            # preview output should not leak goal/step
            preview_r = registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                path="out.txt", content="safe content",
            )
            assert _FILE_INSPECT_SENTINEL_GOAL not in preview_r, f"goal leaked in preview: {preview_r}"
            assert _FILE_INSPECT_SENTINEL_STEP not in preview_r, f"step leaked in preview: {preview_r}"
            assert _FILE_INSPECT_SENTINEL_SECRET not in preview_r, f"secret leaked in preview: {preview_r}"

            # Listing output is bounded and relative
            list_parsed = json.loads(list_r)
            for f in list_parsed["files"]:
                assert not f.startswith("/"), f"absolute path in listing: {f}"
                assert ".." not in f, f"traversal in listing: {f}"

            # Read output is bounded (content size check)
            read_parsed = json.loads(read_r)
            assert read_parsed["size"] <= 64 * 1024, f"size exceeds MAX_FILE_BYTES: {read_parsed['size']}"

            # Error outputs also should not leak sentinels
            err_r = registry.call("read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="nonexistent.txt")
            assert _FILE_INSPECT_SENTINEL_GOAL not in err_r, f"goal leaked in error: {err_r}"
        finally:
            db.close()


def eval_workspace_file_inspection_oversized_binary():
    """Oversized and binary/non-UTF8 files return bounded JSON errors without leaking task sentinels."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            (ws_root / "big.txt").write_text(big_content, encoding="utf-8")
            r_big = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="big.txt",
            ))
            assert "error" in r_big, f"oversized file should error: {r_big}"
            # Must not leak sentinels in error response
            r_big_str = json.dumps(r_big)
            assert _FILE_INSPECT_SENTINEL_GOAL not in r_big_str, f"goal leaked in oversized error: {r_big_str}"
            assert _FILE_INSPECT_SENTINEL_SECRET not in r_big_str, f"secret leaked in oversized error: {r_big_str}"
            assert _FILE_INSPECT_SENTINEL_STEP not in r_big_str, f"step leaked in oversized error: {r_big_str}"

            # Binary/non-UTF8 file
            (ws_root / "binary.bin").write_bytes(b"\x80\x81\x82\xff\xfe\x00\x01\x02")
            r_bin = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="binary.bin",
            ))
            assert "error" in r_bin, f"binary file should error: {r_bin}"
            # Must not leak sentinels in error response
            r_bin_str = json.dumps(r_bin)
            assert _FILE_INSPECT_SENTINEL_GOAL not in r_bin_str, f"goal leaked in binary error: {r_bin_str}"
            assert _FILE_INSPECT_SENTINEL_SECRET not in r_bin_str, f"secret leaked in binary error: {r_bin_str}"
            assert _FILE_INSPECT_SENTINEL_STEP not in r_bin_str, f"step leaked in binary error: {r_bin_str}"
        finally:
            db.close()


def eval_workspace_file_inspection_symlink_sensitive_paths():
    """Symlinks escaping workspace are skipped. Sensitive files (.env, .git) are rejected."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            outside_file.write_text("SECRET_OUTSIDE", encoding="utf-8")
            symlink_path = ws_root / "link_to_outside"
            try:
                ...
                # Symlink should NOT appear in listing (target escapes workspace)
                list_r = json.loads(registry.call("list_worker_workspace_files", worker_id="w_fi", task_id=tid))
                assert "link_to_outside" not in list_r["files"], f"symlink escape listed: {list_r['files']}"

                # Reading symlink should be rejected
                read_r = json.loads(registry.call(
                    "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="link_to_outside",
                ))
                assert "error" in read_r, f"symlink escape read should error: {read_r}"

            # Sensitive files: .env
            (ws_root / ".env").write_text("SECRET_KEY=abc123", encoding="utf-8")
            list_r2 = json.loads(registry.call("list_worker_workspace_files", worker_id="w_fi", task_id=tid))
            assert ".env" not in list_r2["files"], f".env listed: {list_r2['files']}"

            read_env = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path=".env",
            ))
            assert "error" in read_env, f".env read should error: {read_env}"

            preview_env = json.loads(registry.call(
                "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                path=".env", content="NEW_SECRET=xyz",
            ))
            assert "error" in preview_env, f".env preview should error: {preview_env}"

            # Sensitive dirs: .git
            (ws_root / ".git" / "config").parent.mkdir(parents=True, exist_ok=True)
            (ws_root / ".git" / "config").write_text("[core]\n", encoding="utf-8")
            list_r3 = json.loads(registry.call("list_worker_workspace_files", worker_id="w_fi", task_id=tid))
            for f in list_r3["files"]:
                assert not f.startswith(".git/"), f".git file listed: {f}"

            read_git = json.loads(registry.call(
                "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path=".git/config",
            ))
            assert "error" in read_git, f".git read should error: {read_git}"

            # Symlinks pointing to denied directories inside workspace
            # gitlink -> .git/config
            (ws_root / "logs").mkdir(parents=True, exist_ok=True)
            (ws_root / "logs" / "app.log").write_text("log entry\n", encoding="utf-8")
            gitlink_path = ws_root / "gitlink"
            loglink_path = ws_root / "loglink"
            try:
                gitlink_path.symlink_to(ws_root / ".git" / "config")
                loglink_path.symlink_to(ws_root / "logs" / "app.log")
            except OSError:
                pass  # skip if symlinks not supported
            else:
                # list should skip symlinks whose resolved target is under denied dirs
                list_r4 = json.loads(registry.call("list_worker_workspace_files", worker_id="w_fi", task_id=tid))
                assert "gitlink" not in list_r4["files"], f"gitlink (-> .git/config) listed: {list_r4['files']}"
                assert "loglink" not in list_r4["files"], f"loglink (-> logs/app.log) listed: {list_r4['files']}"

                # read should reject symlinks to denied dirs
                read_gitlink = json.loads(registry.call(
                    "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="gitlink",
                ))
                assert "error" in read_gitlink, f"gitlink read should error: {read_gitlink}"

                read_loglink = json.loads(registry.call(
                    "read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="loglink",
                ))
                assert "error" in read_loglink, f"loglink read should error: {read_loglink}"

                # preview should reject symlinks to denied dirs
                prev_gitlink = json.loads(registry.call(
                    "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                    path="gitlink", content="x",
                ))
                assert "error" in prev_gitlink, f"gitlink preview should error: {prev_gitlink}"

                prev_loglink = json.loads(registry.call(
                    "preview_worker_workspace_write", worker_id="w_fi", task_id=tid,
                    path="loglink", content="x",
                ))
                assert "error" in prev_loglink, f"loglink preview should error: {prev_loglink}"
        finally:
            db.close()


def eval_workspace_file_inspection_compatibility():
    """File inspection and preview do not mutate task/worker/lease/event state. Error calls don't break other tools."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            before_status = before_task.status
            before_worker = ws.get_worker("w_fi")
            before_worker_status = before_worker.status
            before_events = es.list_events(task_id=tid)
            before_event_count = len(before_events)

            # Call all three tools
            registry.call("list_worker_workspace_files", worker_id="w_fi", task_id=tid)
            registry.call("read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="README.md")
            registry.call("preview_worker_workspace_write", worker_id="w_fi", task_id=tid, path="new.py", content="x")
            ...
            after_worker = ws.get_worker("w_fi")
            assert after_worker.status == before_worker_status, f"worker status mutated"

            # Event state unchanged
            after_events = es.list_events(task_id=tid)
            assert len(after_events) == before_event_count, f"event count mutated: {before_event_count} -> {len(after_events)}"

            # Error calls don't break existing tools
            registry.call("read_worker_workspace_file", worker_id="w_fi", task_id=tid, path="nonexistent.txt")
            registry.call("read_worker_workspace_file", worker_id="w_ghost", task_id="dtask_x", path="f.txt")

            # Worker/task registry tools still work
            assert "error" not in json.loads(registry.call("get_worker", worker_id="w_fi"))
            assert "error" not in json.loads(registry.call("get_durable_task", task_id=tid))
            assert isinstance(json.loads(registry.call("list_workers")), list)
            assert isinstance(json.loads(registry.call("list_durable_tasks")), list)

            # Workspace lease tools still work
            gw = json.loads(registry.call("get_worker_workspace", worker_id="w_fi", task_id=tid))
            assert "lease_id" in gw, f"get_worker_workspace broken after file inspection: {gw}"

            # Sandbox guard tools still work
            ws_root = Path(ws_path)
            valid_path = str(ws_root / "src" / "main.py")
            vp = json.loads(registry.call("validate_worker_workspace_path", worker_id="w_fi", task_id=tid, path=valid_path))
            assert vp.get("valid") is True, f"validate broken after file inspection: {vp}"

            # Claim/dispatch still work after file inspection
            # Create a pending task and idle worker, then claim
            ws.register_worker("w_claimer", role="coder")
            claim_task = ts.create_task(goal="claim test", steps=[{"text": "s"}])
            claim_r = json.loads(registry.call("claim_durable_task", worker_id="w_claimer"))
            assert "task_id" in claim_r, f"claim broken after file inspection: {claim_r}"
            assert "workspace" in claim_r, f"claim missing workspace after file inspection: {claim_r}"

            # Create another idle worker + pending task, then dispatch
            ws.register_worker("w_dispatch", role="coder")
            ts.create_task(goal="dispatch test", steps=[{"text": "s"}])
...
# --- TASK-082: Finalize merge eval sentinels ---
_FINALIZE_SENTINEL_GOAL = "NORA_EVAL_FINALIZE_GOAL_SENTINEL_f1n2a3l4"
_FINALIZE_SENTINEL_SECRET = "NORA_EVAL_FINALIZE_SECRET_sk-finalize-i5z6e7"
_FINALIZE_SENTINEL_STEP = "NORA_EVAL_FINALIZE_STEP_SENTINEL_d8u9a0l1"
_FINALIZE_SENTINEL_FILE = "NORA_EVAL_FINALIZE_FILE_SECRET_s2e3c4r5"
_FINALIZE_SENTINEL_ENV = "NORA_EVAL_FINALIZE_ENV_SECRET_e6t7_s8e9"


def _setup_finalize_worker(registry, worker_id="w_fin"):
    """Helper for finalize evals: register worker, create task with sentinels, assign, set running, prepare workspace lease.
    Returns (task_id, workspace_path, lease_id)."""
    ws = registry.durable_worker_store
    ts = registry.durable_task_store
    ws.register_worker(worker_id, role="coder")
    task = ts.create_task(
        goal=_FINALIZE_SENTINEL_GOAL + " " + _FINALIZE_SENTINEL_SECRET,
        steps=[{"text": _FINALIZE_SENTINEL_STEP}],
    )
    tid = task.task_id
    ts.assign_worker(tid, worker_id)
    ws.update_status(worker_id, "assigned", current_task_id=tid)
...
def _setup_and_finalize(registry, worker_id="w_fin", files=None, release_workspace=True):
    """Helper: setup worker, create files, record approved gate, apply merge, finalize.
    Returns (task_id, workspace_path, lease_id, finalize_result)."""
    tid, ws_path, lease_id = _setup_finalize_worker(registry, worker_id=worker_id)
    ws_root = Path(ws_path)
    if files:
        for name, content in files.items():
            (ws_root / name).write_text(content, encoding="utf-8")
    ...
    assert apply_result["applied"] is True, f"setup apply failed: {apply_result}"
    finalize_result = json.loads(registry.call(
        "finalize_worker_workspace_merge", worker_id=worker_id, task_id=tid,
        release_workspace=release_workspace,
    ))
    return tid, ws_path, lease_id, finalize_result


def eval_finalize_merge_successful_finalization():
    """Successful finalization: task completed, worker idle, lease released, safe status metadata."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            tid, ws_path, lease_id = _setup_finalize_worker(registry)
            ws_root = Path(ws_path)
            (ws_root / "hello.txt").write_text("hello world", encoding="utf-8")
            _record_gate(registry, "w_fin", tid, "approved")
            registry.call("apply_reviewed_worker_workspace_merge", worker_id="w_fin", task_id=tid)

            # Snapshot before
            task_before = ts.get_task(tid)
            worker_before = ws.get_worker("w_fin")

            # Finalize with release
            result = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_fin", task_id=tid, release_workspace=True,
            ))
            assert result["finalized"] is True, f"expected finalized=True: {result}"
            assert result["task_id"] == tid
            assert result["worker_id"] == "w_fin"
            assert result.get("task_status_before") == "running"
            assert result.get("task_status_after") == "completed"
            assert result.get("worker_status_before") in ("assigned", "running")
            assert result.get("worker_status_after") == "idle"
            assert result.get("workspace_released") is True
            assert result.get("lease_id") == lease_id

            # Task is completed
            task_after = ts.get_task(tid)
            assert task_after.status == "completed", f"task status: {task_after.status}"

            # Worker is idle, current_task_id cleared
            worker_after = ws.get_worker("w_fin")
            assert worker_after.status == "idle", f"worker status: {worker_after.status}"
            assert worker_after.current_task_id is None, f"current_task_id not cleared: {worker_after.current_task_id}"

            # Lease released
            lease_after = registry.workspace_lease_store.get_lease_by_worker("w_fin")
            assert lease_after is None, f"lease should be released: {lease_after}"
...
def eval_finalize_merge_guard_rails():
    """Reject before apply, reject missing lease, reject stale apply event, reject invalid release_workspace, release_workspace=False keeps lease, repeated finalization."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            # Reject before successful apply (no_successful_apply)
            tid, ws_path, lease_id = _setup_finalize_worker(registry, worker_id="w_guard")
            (Path(ws_path) / "f.txt").write_text("content", encoding="utf-8")
            _record_gate(registry, "w_guard", tid, "approved")
            # Don't apply, just finalize
            r1 = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_guard", task_id=tid,
            ))
            assert r1["finalized"] is False
            assert r1.get("reason") == "no_successful_apply", f"reason: {r1}"

            # Reject when active lease is missing (after releasing)
            tid2, ws_path2, lease_id2 = _setup_finalize_worker(registry, worker_id="w_guard2")
            (Path(ws_path2) / "guard2.txt").write_text("content", encoding="utf-8")
            _record_gate(registry, "w_guard2", tid2, "approved")
            apply2 = json.loads(registry.call("apply_reviewed_worker_workspace_merge", worker_id="w_guard2", task_id=tid2))
            assert apply2["applied"] is True, f"guard2 apply failed: {apply2}"
            # Release lease manually
            registry.workspace_lease_store.release_lease(lease_id2)
            r2 = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_guard2", task_id=tid2,
            ))
            assert r2["finalized"] is False
            assert r2.get("reason") == "workspace_lease_invalid", f"reason: {r2}"

            # Reject stale apply event from a previous lease, even if the lease id is reused
            replacement = json.loads(registry.call("prepare_worker_workspace", worker_id="w_guard2", task_id=tid2))
            assert replacement["lease_id"] == lease_id2, f"expected reused lease id in this store setup: {replacement}"
            r2b = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_guard2", task_id=tid2,
            ))
            assert r2b["finalized"] is False
            assert r2b.get("reason") == "no_successful_apply", f"reason: {r2b}"

            # release_workspace=False completes task/worker but keeps lease
            tid3, ws_path3, lease_id3 = _setup_finalize_worker(registry, worker_id="w_guard3")
            (Path(ws_path3) / "guard3.txt").write_text("content", encoding="utf-8")
            _record_gate(registry, "w_guard3", tid3, "approved")
            apply3 = json.loads(registry.call("apply_reviewed_worker_workspace_merge", worker_id="w_guard3", task_id=tid3))
            assert apply3["applied"] is True, f"guard3 apply failed: {apply3}"
            r3 = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_guard3", task_id=tid3, release_workspace=False,
            ))
            assert r3["finalized"] is True
            assert r3.get("workspace_released") is False
            # Lease still exists
            lease3 = registry.workspace_lease_store.get_lease_by_worker("w_guard3")
            assert lease3 is not None, f"lease should still exist: {lease3}"
            # But task is completed and worker is idle
            assert ts.get_task(tid3).status == "completed"
            assert ws.get_worker("w_guard3").status == "idle"

            # Repeated finalization returns already_finalized
            r4 = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_guard3", task_id=tid3,
            ))
            assert r4["finalized"] is False
            assert r4.get("reason") == "already_finalized", f"reason: {r4}"

            # Task not running (already completed from above)
            # Create a new task that is pending (not running)
            ws.register_worker("w_guard4", role="coder")
            pending_task = ts.create_task(goal="pending", steps=[{"text": "x"}])
            ts.assign_worker(pending_task.task_id, "w_guard4")
            ws.update_status("w_guard4", "assigned", current_task_id=pending_task.task_id)
            # Don't set to running
            r5 = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_guard4", task_id=pending_task.task_id,
            ))
            assert r5["finalized"] is False
            assert r5.get("reason") == "task_not_running", f"reason: {r5}"
        finally:
            db.close()


def eval_finalize_merge_safety_no_leak():
    """Output and events do not leak sentinels. Error outputs bounded."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)

            # Successful finalize with sentinels
            tid, ws_path, lease_id = _setup_finalize_worker(registry)
            (Path(ws_path) / "secret.txt").write_text(_FINALIZE_SENTINEL_FILE, encoding="utf-8")
            _record_gate(registry, "w_fin", tid, "approved")
            registry.call("apply_reviewed_worker_workspace_merge", worker_id="w_fin", task_id=tid)
            result = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_fin", task_id=tid,
            ))
            result_str = json.dumps(result)
            assert _FINALIZE_SENTINEL_GOAL not in result_str, f"goal leaked: {result_str[:200]}"
            assert _FINALIZE_SENTINEL_SECRET not in result_str, f"secret leaked: {result_str[:200]}"
            assert _FINALIZE_SENTINEL_STEP not in result_str, f"step leaked: {result_str[:200]}"
            assert _FINALIZE_SENTINEL_FILE not in result_str, f"file content leaked: {result_str[:200]}"

            # Error output bounded
            bad = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="nonexistent", task_id=tid,
            ))
            assert "error" in bad or bad.get("finalized") is False
            bad_str = json.dumps(bad)
            assert _FINALIZE_SENTINEL_GOAL not in bad_str, f"goal leaked in error: {bad_str}"
            assert _FINALIZE_SENTINEL_SECRET not in bad_str, f"secret leaked in error: {bad_str}"

            # Event payload safe
            events = registry.durable_event_store.list_events(task_id=tid, event_type="task_status_changed", max_results=10)
            for event in events:
                payload_str = json.dumps(event.payload)
                assert _FINALIZE_SENTINEL_GOAL not in payload_str, f"goal leaked in event: {payload_str}"
                assert _FINALIZE_SENTINEL_SECRET not in payload_str, f"secret leaked in event: {payload_str}"
                assert _FINALIZE_SENTINEL_STEP not in payload_str, f"step leaked in event: {payload_str}"
        finally:
            db.close()


def eval_finalize_merge_no_unintended_mutation():
    """No project/workspace deletion. Rejection paths don't mutate task/worker/lease."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            ws_root = Path(ws_path)
            project_root = Path(tmpdir)
            (ws_root / "hello.txt").write_text("hello", encoding="utf-8")
            (project_root / "project_file.txt").write_text("project", encoding="utf-8")
            _record_gate(registry, "w_fin", tid, "approved")
            registry.call("apply_reviewed_worker_workspace_merge", worker_id="w_fin", task_id=tid)

            # Finalize
            registry.call("finalize_worker_workspace_merge", worker_id="w_fin", task_id=tid)

            # Project root not deleted
            assert project_root.exists(), "project root deleted"
            assert (project_root / "project_file.txt").exists(), "project file deleted"

            # Workspace not deleted (lease released but dir may still exist)
            assert ws_root.exists(), "workspace deleted"

            # Now test rejection path doesn't mutate
            ws.register_worker("w_fin_reject", role="coder")
            reject_task = ts.create_task(goal="reject test", steps=[{"text": "x"}])
            ts.assign_worker(reject_task.task_id, "w_fin_reject")
            ws.update_status("w_fin_reject", "assigned", current_task_id=reject_task.task_id)
            ts.update_status(reject_task.task_id, "running")
            # Prepare workspace for this worker too
            registry.call("prepare_worker_workspace", worker_id="w_fin_reject", task_id=reject_task.task_id)

            # Snapshot
            task_before = ts.get_task(reject_task.task_id)
            worker_before = ws.get_worker("w_fin_reject")

            # Finalize without apply (should fail)
            r = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_fin_reject", task_id=reject_task.task_id,
            ))
            assert r["finalized"] is False

            # State unchanged
            task_after = ts.get_task(reject_task.task_id)
            worker_after = ws.get_worker("w_fin_reject")
...
def eval_finalize_merge_compatibility():
    """Existing tools still work after successful finalization. Failed finalize doesn't break tools."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            audit = json.loads(registry.call("list_worker_workspace_merge_applies", worker_id="w_fin", task_id=tid))
            ...
            worker = json.loads(registry.call("get_worker", worker_id="w_fin"))
            assert worker["worker_id"] == "w_fin"
            assert worker["status"] == "idle"

            # Claim/dispatch still work
            ws.register_worker("w_fin_claim", role="coder")
            claim_task = ts.create_task(goal="claim after finalize", steps=[{"text": "x"}])
            claim = json.loads(registry.call("claim_durable_task", worker_id="w_fin_claim"))
            assert claim.get("task_id") == claim_task.task_id
            ws.register_worker("w_fin_dispatch", role="coder")
            ts.create_task(goal="dispatch after finalize", steps=[{"text": "x"}])
            dispatch = json.loads(registry.call("dispatch_durable_tasks"))
            assert "dispatched" in dispatch

            # Failed finalize doesn't break tools
            ws.register_worker("w_fin_fail", role="coder")
            fail_task = ts.create_task(goal="fail test", steps=[{"text": "x"}])
            ts.assign_worker(fail_task.task_id, "w_fin_fail")
            ws.update_status("w_fin_fail", "assigned", current_task_id=fail_task.task_id)
            ts.update_status(fail_task.task_id, "running")
            fail_result = json.loads(registry.call(
                "finalize_worker_workspace_merge", worker_id="w_fin_fail", task_id=fail_task.task_id,
            ))
            assert fail_result["finalized"] is False
...
_LIFECYCLE_SENTINEL_FAILURE = "NORA_EVAL_LIFECYCLE_FAILURE_REASON_SENTINEL_f1a2i3l4"
...
def _setup_failed_task(registry, worker_id="w_fail", task_id=None, retry_count=0, max_retries=3, goal=None, steps=None, set_worker_idle=True, failure_reason=None):
    """Helper: create a failed task with specific retry_count. Returns (task_id, worker_id).

    By default sets worker idle after failure so task is retryable (no active worker attached).
    Pass set_worker_idle=False to keep worker assigned (for blocked_active_worker tests).
    """
    ws = registry.durable_worker_store
    ts = registry.durable_task_store
    ws.register_worker(worker_id, role="coder")
    _goal = goal or _LIFECYCLE_SENTINEL_GOAL + " " + _LIFECYCLE_SENTINEL_SECRET
    _steps = steps or [{"text": _LIFECYCLE_SENTINEL_STEP}]
    _failure = failure_reason or _LIFECYCLE_SENTINEL_FAILURE
    task = ts.create_task(goal=_goal, steps=_steps, max_retries=max_retries)
    tid = task_id or task.task_id
    ts.assign_worker(tid, worker_id)
    ws.update_status(worker_id, "assigned", current_task_id=tid)
    ts.update_status(tid, "running")
    ts.update_status(tid, "failed", failure_reason=_failure)
    # Manually set retry_count by re-reading and updating
    # The retry_durable_task API increments retry_count and resets to pending
    for _ in range(retry_count):
        ts.retry_durable_task(tid)
        ts.update_status(tid, "running")
        ts.update_status(tid, "failed", failure_reason=_failure)
    if set_worker_idle:
        ws.update_status(worker_id, "idle", current_task_id="")
    return tid, worker_id


def eval_retry_planner_available():
    """Failed task with retries remaining is surfaced as retry_failed_task / retry_available."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_avail", retry_count=0, max_retries=3)
            result = json.loads(registry.call("plan_worker_lifecycle_actions"))
            retry_actions = [a for a in result["actions"] if a.get("action") == "retry_failed_task"]
            assert len(retry_actions) >= 1, f"no retry action: {result['actions']}"
            assert retry_actions[0]["task_id"] == tid
            assert retry_actions[0]["reason"] == "retry_available"
            assert retry_actions[0]["retry_count"] == 0
            assert retry_actions[0]["max_retries"] == 3
        finally:
            db.close()


def eval_retry_planner_exhausted():
    """Failed task with retry_count >= max_retries is not recommended for retry."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_exh", retry_count=3, max_retries=3)
            result = json.loads(registry.call("plan_worker_lifecycle_actions"))
            retry_actions = [a for a in result["actions"] if a.get("action") == "retry_failed_task" and a.get("task_id") == tid]
            assert len(retry_actions) == 0, f"exhausted task should not have retry action: {retry_actions}"
            assert result["summary"]["retry_exhausted"] >= 1
        finally:
            db.close()


def eval_retry_planner_blocked_active_worker():
    """Failed task with active ASSIGNED or RUNNING owner worker is blocked."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            # Case 1: ASSIGNED owner
            tid1, wid1 = _setup_failed_task(registry, worker_id="w_retry_blk_a", retry_count=0, max_retries=3, set_worker_idle=False)
            result1 = json.loads(registry.call("plan_worker_lifecycle_actions"))
            retry_actions1 = [a for a in result1["actions"] if a.get("action") == "retry_failed_task" and a.get("task_id") == tid1]
            assert len(retry_actions1) == 0, f"ASSIGNED owner: should not have retry action: {retry_actions1}"
            assert result1["summary"]["retry_blocked_active_worker"] >= 1
            # Case 2: RUNNING owner — set worker to running
            tid2, wid2 = _setup_failed_task(registry, worker_id="w_retry_blk_r", retry_count=0, max_retries=3, set_worker_idle=False)
            ws.update_status(wid2, "running", current_task_id=tid2)
            result2 = json.loads(registry.call("plan_worker_lifecycle_actions"))
            retry_actions2 = [a for a in result2["actions"] if a.get("action") == "retry_failed_task" and a.get("task_id") == tid2]
            assert len(retry_actions2) == 0, f"RUNNING owner: should not have retry action: {retry_actions2}"
            assert result2["summary"]["retry_blocked_active_worker"] >= 1
        finally:
            db.close()


def eval_retry_explain_available():
    """Explain: failed task with retries remaining and idle capacity shows retry_available."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_exp", retry_count=1, max_retries=3)
            # Set worker idle to provide capacity
            ws.update_status("w_retry_exp", "idle", current_task_id="")
            result = json.loads(registry.call("explain_worker_lifecycle_scheduler_state"))
            retry_reasons = [r for r in result["blocked_reasons"] if r.get("reason") == "retry_available" and r.get("task_id") == tid]
            assert len(retry_reasons) >= 1, f"no retry_available reason: {result['blocked_reasons']}"
            assert "retry 2/3" in retry_reasons[0]["detail"]
            retry_actions = [a for a in result["next_actions"] if a.get("action") == "retry_failed_task" and a.get("task_id") == tid]
            assert len(retry_actions) >= 1, f"no retry action: {result['next_actions']}"
        finally:
            db.close()


def eval_retry_explain_exhausted():
    """Explain: exhausted retry task shows retry_exhausted."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_exh2", retry_count=3, max_retries=3)
            result = json.loads(registry.call("explain_worker_lifecycle_scheduler_state"))
            retry_reasons = [r for r in result["blocked_reasons"] if r.get("reason") == "retry_exhausted" and r.get("task_id") == tid]
            assert len(retry_reasons) >= 1, f"no retry_exhausted reason: {result['blocked_reasons']}"
            assert "max retries" in retry_reasons[0]["detail"]
        finally:
            db.close()


def eval_retry_explain_blocked_active_worker():
    """Explain: failed task with active ASSIGNED or RUNNING worker shows retry_blocked_active_worker."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ws = registry.durable_worker_store
            # Case 1: ASSIGNED owner
            tid1, wid1 = _setup_failed_task(registry, worker_id="w_retry_blk2a", retry_count=0, max_retries=3, set_worker_idle=False)
            result1 = json.loads(registry.call("explain_worker_lifecycle_scheduler_state"))
            retry_reasons1 = [r for r in result1["blocked_reasons"] if r.get("reason") == "retry_blocked_active_worker" and r.get("task_id") == tid1]
            assert len(retry_reasons1) >= 1, f"ASSIGNED: no retry_blocked_active_worker: {result1['blocked_reasons']}"
            assert retry_reasons1[0]["worker_id"] == wid1
            assert "active" in retry_reasons1[0]["detail"].lower()
            # Case 2: RUNNING owner
            tid2, wid2 = _setup_failed_task(registry, worker_id="w_retry_blk2r", retry_count=0, max_retries=3, set_worker_idle=False)
            ws.update_status(wid2, "running", current_task_id=tid2)
            result2 = json.loads(registry.call("explain_worker_lifecycle_scheduler_state"))
            retry_reasons2 = [r for r in result2["blocked_reasons"] if r.get("reason") == "retry_blocked_active_worker" and r.get("task_id") == tid2]
            assert len(retry_reasons2) >= 1, f"RUNNING: no retry_blocked_active_worker: {result2['blocked_reasons']}"
            assert retry_reasons2[0]["worker_id"] == wid2
            assert "active" in retry_reasons2[0]["detail"].lower()
        finally:
            db.close()


def eval_retry_explain_missing_capacity():
    """Explain: retryable task with no idle workers shows retry_blocked_missing_capacity."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_cap", retry_count=0, max_retries=3)
            # Set worker offline to remove capacity
            ws.update_status("w_retry_cap", "offline", current_task_id="")
            result = json.loads(registry.call("explain_worker_lifecycle_scheduler_state"))
            retry_reasons = [r for r in result["blocked_reasons"] if r.get("reason") == "retry_blocked_missing_capacity" and r.get("task_id") == tid]
            assert len(retry_reasons) >= 1, f"no retry_blocked_missing_capacity: {result['blocked_reasons']}"
            assert "no idle workers" in retry_reasons[0]["detail"]
        finally:
            db.close()


def eval_retry_priority_vs_closeout():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            # Setup ready closeout
            tid_close, _, _ = _setup_lifecycle_ready_worker(registry, worker_id="w_close_retry")
            # Setup failed task with retries remaining
            tid_fail, _ = _setup_failed_task(registry, worker_id="w_fail_retry", retry_count=0, max_retries=3)
            # Set failed worker idle to make it retryable
            ws.update_status("w_fail_retry", "idle", current_task_id="")
            result = json.loads(registry.call("plan_worker_lifecycle_actions"))
            actions = result["actions"]
            # Find indices
            closeout_idx = next((i for i, a in enumerate(actions) if a.get("action") == "finalize_ready_workspace_merge"), None)
            retry_idx = next((i for i, a in enumerate(actions) if a.get("action") == "retry_failed_task"), None)
            assert closeout_idx is not None, f"no closeout action: {actions}"
            assert retry_idx is not None, f"no retry action: {actions}"
            assert closeout_idx < retry_idx, f"closeout should come before retry: {actions}"
        finally:
            db.close()


def eval_retry_priority_vs_dispatch():
    """Retry comes before pending-task dispatch in planner output."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            # Setup failed task with retries
            tid_fail, _ = _setup_failed_task(registry, worker_id="w_fail_disp", retry_count=0, max_retries=3)
            ws.update_status("w_fail_disp", "idle", current_task_id="")
            # Setup pending task
            ts.create_task(goal="pending dispatch", steps=[{"text": "x"}])
            result = json.loads(registry.call("plan_worker_lifecycle_actions"))
            actions = result["actions"]
            retry_idx = next((i for i, a in enumerate(actions) if a.get("action") == "retry_failed_task"), None)
            dispatch_idx = next((i for i, a in enumerate(actions) if a.get("action") == "dispatch_pending_task"), None)
            assert retry_idx is not None, f"no retry action: {actions}"
            assert dispatch_idx is not None, f"no dispatch action: {actions}"
            assert retry_idx < dispatch_idx, f"retry should come before dispatch: {actions}"
        finally:
            db.close()


def eval_retry_filter_no_leak():
    """task_id and worker_id filters do not leak unrelated retry entries."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            ts = registry.durable_task_store
            # Two failed tasks
            tid1, _ = _setup_failed_task(registry, worker_id="w_f1", retry_count=0, max_retries=3, goal="task 1 goal")
            tid2, _ = _setup_failed_task(registry, worker_id="w_f2", retry_count=0, max_retries=3, goal="task 2 goal")
            ws.update_status("w_f1", "idle", current_task_id="")
            ws.update_status("w_f2", "idle", current_task_id="")
            # Filter by tid1: must not see tid2 or w_f2
            r1 = json.loads(registry.call("explain_worker_lifecycle_scheduler_state", task_id=tid1))
            r1_str = json.dumps(r1)
            assert tid2 not in r1_str, f"{tid2} leaked in {tid1} filter"
            assert "w_f2" not in r1_str, f"w_f2 leaked in {tid1} filter"
            retry_reasons = [r for r in r1["blocked_reasons"] if r.get("reason") == "retry_available"]
            assert len(retry_reasons) == 1
            assert retry_reasons[0]["task_id"] == tid1
            # Filter by w_f1: must not see w_f2, tid2, or unrelated task_id
            r2 = json.loads(registry.call("explain_worker_lifecycle_scheduler_state", worker_id="w_f1"))
            r2_str = json.dumps(r2)
            assert "w_f2" not in r2_str, f"w_f2 leaked in w_f1 filter"
            assert tid2 not in r2_str, f"{tid2} leaked in w_f1 filter"
            # Retry entries have empty worker_id, so they should be filtered out by worker_id filter
            retry_reasons2 = [r for r in r2["blocked_reasons"] if r.get("reason") == "retry_available"]
            assert len(retry_reasons2) == 0, f"retry entries should not appear with worker_id filter: {retry_reasons2}"
        finally:
            db.close()


def eval_retry_read_only_no_mutation():
    """Planner and explain calls do not mutate task status, retry_count, worker_id, or worker state."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            tid, wid = _setup_failed_task(registry, worker_id="w_retry_mut", retry_count=1, max_retries=3)
            # Snapshot task state
            task_before = ts.get_task(tid)
            worker_before = ws.get_worker(wid)
            task_status_before = task_before.status
            retry_count_before = task_before.retry_count
            task_worker_before = task_before.worker_id
            worker_status_before = worker_before.status if isinstance(worker_before.status, str) else worker_before.status.value
            worker_task_before = worker_before.current_task_id
            # Call planner
            registry.call("plan_worker_lifecycle_actions")
            task_after_plan = ts.get_task(tid)
            worker_after_plan = ws.get_worker(wid)
            assert task_after_plan.status == task_status_before, f"task status changed after planner: {task_after_plan.status}"
            assert task_after_plan.retry_count == retry_count_before, f"retry_count changed after planner: {task_after_plan.retry_count}"
            assert task_after_plan.worker_id == task_worker_before, f"task worker_id changed after planner: {task_after_plan.worker_id}"
            worker_status_after_plan = worker_after_plan.status if isinstance(worker_after_plan.status, str) else worker_after_plan.status.value
            assert worker_status_after_plan == worker_status_before, f"worker status changed after planner: {worker_status_after_plan}"
            assert worker_after_plan.current_task_id == worker_task_before, f"worker current_task_id changed after planner: {worker_after_plan.current_task_id}"
            # Call explain
            registry.call("explain_worker_lifecycle_scheduler_state")
            task_after_explain = ts.get_task(tid)
            worker_after_explain = ws.get_worker(wid)
            assert task_after_explain.status == task_status_before, f"task status changed after explain: {task_after_explain.status}"
            assert task_after_explain.retry_count == retry_count_before, f"retry_count changed after explain: {task_after_explain.retry_count}"
            assert task_after_explain.worker_id == task_worker_before, f"task worker_id changed after explain: {task_after_explain.worker_id}"
            worker_status_after_explain = worker_after_explain.status if isinstance(worker_after_explain.status, str) else worker_after_explain.status.value
            assert worker_status_after_explain == worker_status_before, f"worker status changed after explain: {worker_status_after_explain}"
            assert worker_after_explain.current_task_id == worker_task_before, f"worker current_task_id changed after explain: {worker_after_explain.current_task_id}"
        finally:
            db.close()


def eval_retry_safety_no_leak():
    """Planner/explain output does not leak goals, steps, failure reasons, secrets, or workspace paths."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_safe", retry_count=0, max_retries=3)
            # Planner
            plan_str = registry.call("plan_worker_lifecycle_actions")
            assert _LIFECYCLE_SENTINEL_GOAL not in plan_str, "goal leaked in planner"
            assert _LIFECYCLE_SENTINEL_SECRET not in plan_str, "secret leaked in planner"
            assert _LIFECYCLE_SENTINEL_STEP not in plan_str, "step leaked in planner"
            assert _LIFECYCLE_SENTINEL_FAILURE not in plan_str, "failure_reason leaked in planner"
            assert ".workspaces" not in plan_str, "workspace path leaked in planner"
            # Explain
            explain_str = registry.call("explain_worker_lifecycle_scheduler_state")
            assert _LIFECYCLE_SENTINEL_GOAL not in explain_str, "goal leaked in explain"
            assert _LIFECYCLE_SENTINEL_SECRET not in explain_str, "secret leaked in explain"
            assert _LIFECYCLE_SENTINEL_STEP not in explain_str, "step leaked in explain"
            assert _LIFECYCLE_SENTINEL_FAILURE not in explain_str, "failure_reason leaked in explain"
            assert ".workspaces" not in explain_str, "workspace path leaked in explain"
        finally:
            db.close()


def eval_retry_compatibility():
    """Existing tools still work after retry planning/explain calls."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            tid, _ = _setup_failed_task(registry, worker_id="w_retry_compat", retry_count=0, max_retries=3)
            registry.call("plan_worker_lifecycle_actions")
            registry.call("explain_worker_lifecycle_scheduler_state")
            ...
            assert "blocked_reasons" in explain
            ...
            ws = registry.durable_worker_store
            ts = registry.durable_task_store
            ws.register_worker("w_after_retry", role="coder")
            assert ws.get_worker("w_after_retry") is not None
            new_task = ts.create_task(goal="after retry task", steps=[{"text": "x"}])
            assert new_task.task_id is not None
...
def eval_policy_hook_summary_counts():
    """summarize_runtime_policy_hook_evaluations returns correct summary counts for decisions, hooks, categories, risks, confirmation, blocked, policy versions, and recent event IDs."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations"))
            assert result["total"] == 3, f"expected 3 total, got {result['total']}"

            # Decision counts: pre_tool=allow, pre_shell=confirm, pre_git=confirm
            assert result["decisions"]["allow"] == 1, f"expected 1 allow, got {result['decisions']['allow']}"
            assert result["decisions"]["confirm"] == 2, f"expected 2 confirm, got {result['decisions']['confirm']}"
            assert result["decisions"]["block"] == 0, f"expected 0 block, got {result['decisions']['block']}"

            # Hook counts
            assert result["hooks"]["pre_tool"] == 1, f"expected 1 pre_tool, got {result['hooks'].get('pre_tool', 0)}"
            assert result["hooks"]["pre_shell"] == 1, f"expected 1 pre_shell, got {result['hooks'].get('pre_shell', 0)}"
            assert result["hooks"]["pre_git"] == 1, f"expected 1 pre_git, got {result['hooks'].get('pre_git', 0)}"

            # Category counts
            assert result["categories"]["file"] == 1, f"expected 1 file, got {result['categories'].get('file', 0)}"
            assert result["categories"]["shell"] == 1, f"expected 1 shell, got {result['categories'].get('shell', 0)}"
            assert result["categories"]["git"] == 1, f"expected 1 git, got {result['categories'].get('git', 0)}"

            # Risk counts
            assert result["risks"]["read"] == 1, f"expected 1 read, got {result['risks'].get('read', 0)}"
            assert result["risks"]["write"] == 2, f"expected 2 write, got {result['risks'].get('write', 0)}"

            # Confirmation and blocked counts
            assert result["requires_confirmation_count"] == 2, f"expected 2 confirmations, got {result['requires_confirmation_count']}"
            assert result["blocked_count"] == 0, f"expected 0 blocked, got {result['blocked_count']}"

            # Policy versions
            assert len(result["policy_versions"]) >= 1, "expected at least 1 policy version"

            # Recent event IDs
            assert len(result["recent_event_ids"]) == 3, f"expected 3 recent event IDs, got {len(result['recent_event_ids'])}"
            recorded_ids = {r["event_id"] for r in recorded}
            summary_ids = set(result["recent_event_ids"])
            assert recorded_ids == summary_ids, f"event IDs mismatch: {recorded_ids} vs {summary_ids}"
        finally:
            db.close()


def eval_policy_hook_summary_filter_hook():
    """Hook filter returns summary only for matching events."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", hook="pre_shell"))
            assert result["total"] == 1, f"expected 1 event for pre_shell, got {result['total']}"
            assert result["hooks"] == {"pre_shell": 1}, f"expected only pre_shell hook, got {result['hooks']}"
            assert result["filters"]["hook"] == "pre_shell"
        finally:
            db.close()


def eval_policy_hook_summary_filter_decision():
    """Decision filter returns summary only for matching events."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", decision="allow"))
            assert result["total"] == 1, f"expected 1 allow event, got {result['total']}"
            assert result["decisions"] == {"allow": 1, "confirm": 0, "block": 0}, f"expected only allow decision, got {result['decisions']}"
            assert result["filters"]["decision"] == "allow"

            result2 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", decision="confirm"))
            assert result2["total"] == 2, f"expected 2 confirm events, got {result2['total']}"
            assert result2["decisions"]["confirm"] == 2
        finally:
            db.close()


def eval_policy_hook_summary_filter_category_risk():
    """Category and risk filters return summary only for matching events."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            _record_policy_hook_events(registry, count=3)  # file/shell/git, read/write/write

            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", category="file"))
            assert result["total"] == 1, f"expected 1 file event, got {result['total']}"
            assert result["categories"] == {"file": 1}, f"expected only file category, got {result['categories']}"
            assert result["filters"]["category"] == "file"

            result2 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", risk="read"))
            assert result2["total"] == 1, f"expected 1 read event, got {result2['total']}"
            assert result2["risks"] == {"read": 1}, f"expected only read risk, got {result2['risks']}"
            assert result2["filters"]["risk"] == "read"
        finally:
            db.close()


def eval_policy_hook_summary_filter_linkage():
    """task_id, worker_id, session_id filters work correctly."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            ...
            # Filter by task_id
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", task_id="task_001"))
            assert result["total"] == 1, f"expected 1 task_001 event, got {result['total']}"
            assert result["filters"]["task_id"] == "task_001"

            # Filter by worker_id
            result2 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", worker_id="worker_b"))
            assert result2["total"] == 1, f"expected 1 worker_b event, got {result2['total']}"
            assert result2["filters"]["worker_id"] == "worker_b"

            # Filter by session_id
            result3 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", session_id="sess_1"))
            assert result3["total"] == 1, f"expected 1 sess_1 event, got {result3['total']}"
            assert result3["filters"]["session_id"] == "sess_1"
        finally:
            db.close()


def eval_policy_hook_summary_limit():
    """Limit parameter bounds the summary to recent events."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations"))
            assert result["total"] == 5, f"expected 5 events with default limit, got {result['total']}"
            assert result["filters"]["limit"] == 20

            # Limit=2 returns only 2
            result2 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", limit=2))
            assert result2["total"] == 2, f"expected 2 events with limit=2, got {result2['total']}"
            assert result2["filters"]["limit"] == 2
            assert len(result2["recent_event_ids"]) == 2

            # Limit=200 clamps to 100
            result3 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", limit=200))
            assert result3["filters"]["limit"] == 100, f"expected limit clamped to 100, got {result3['filters']['limit']}"

            # Limit=0 clamps to 1
            result4 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", limit=0))
            assert result4["filters"]["limit"] == 1, f"expected limit clamped to 1, got {result4['filters']['limit']}"
        finally:
            db.close()


def eval_policy_hook_summary_invalid_filters():
    """Invalid/unsafe filters return empty bounded output or safe errors without returning all events."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            _record_policy_hook_events(registry, count=3)

            # Invalid hook
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", hook="nonexistent_hook"))
            assert result["total"] == 0, f"expected 0 for invalid hook, got {result['total']}"
            assert "invalid_hook" in result.get("errors", []), f"expected invalid_hook error, got {result}"
            assert result["recent_event_ids"] == [], "expected no event IDs for invalid hook"

            # Invalid decision
            result2 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", decision="invalid_decision"))
            assert result2["total"] == 0, f"expected 0 for invalid decision, got {result2['total']}"
            assert "invalid_decision" in result2.get("errors", []), f"expected invalid_decision error, got {result2}"

            # Invalid category
            result3 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", category="invalid_category"))
            assert result3["total"] == 0, f"expected 0 for invalid category, got {result3['total']}"
            assert "invalid_category" in result3.get("errors", []), f"expected invalid_category error, got {result3}"

            # Invalid risk
            result4 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", risk="invalid_risk"))
            assert result4["total"] == 0, f"expected 0 for invalid risk, got {result4['total']}"
            assert "invalid_risk" in result4.get("errors", []), f"expected invalid_risk error, got {result4}"
        finally:
            db.close()


def eval_policy_hook_summary_unsafe_linkage():
    """Unsafe linkage filters return empty bounded output with errors."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", task_id="/etc/passwd"))
            assert result["total"] == 0, f"expected 0 for unsafe task_id, got {result['total']}"
            assert "invalid_task_id" in result.get("errors", []), f"expected invalid_task_id error, got {result}"

            # Unsafe worker_id (secret-like)
            result2 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", worker_id="SECRET_TOKEN_ABC"))
            assert result2["total"] == 0, f"expected 0 for unsafe worker_id, got {result2['total']}"
            assert "invalid_worker_id" in result2.get("errors", []), f"expected invalid_worker_id error, got {result2}"

            # Unsafe session_id (shell metachar)
            result3 = json.loads(registry.call("summarize_runtime_policy_hook_evaluations", session_id="sess;rm -rf"))
            assert result3["total"] == 0, f"expected 0 for unsafe session_id, got {result3['total']}"
            assert "invalid_session_id" in result3.get("errors", []), f"expected invalid_session_id error, got {result3}"
        finally:
            db.close()


def eval_policy_hook_summary_no_leak():
    """Raw reason/action/linkage sentinels, shell commands, env strings, workspace paths, and secrets do not leak."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            registry.call("record_runtime_policy_hook_evaluation", hook="pre_tool", action="SECRET_TOKEN_ABCDEF")
            registry.call("record_runtime_policy_hook_evaluation", hook="pre_tool", action="rm -rf /")
            registry.call("record_runtime_policy_hook_evaluation", hook="pre_tool", action="DATABASE_URL=secret")
            registry.call("record_runtime_policy_hook_evaluation", hook="pre_tool", task_id="task_with_secret", reason="RAW_REASON_SENTINEL_XYZ_789")

            result_str = registry.call("summarize_runtime_policy_hook_evaluations")
...
def eval_policy_hook_summary_read_only_no_mutation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # Register a worker to verify no mutation
            registry.call("register_worker", worker_id="test_worker_summary_001", role="coder")
            worker_before = ws.get_worker("test_worker_summary_001")
            assert worker_before is not None, "worker not registered"
            ...
            # Summary should not create events or mutate state
            result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations"))
            ...
            after_tasks = len(ts.list_tasks())
            after_workers = len(ws.list_workers())
            worker_after = ws.get_worker("test_worker_summary_001")
            assert after_event_count == before_event_count, f"summary created events: {before_event_count} -> {after_event_count}"
            assert after_tasks == before_tasks, f"summary mutated tasks: {before_tasks} -> {after_tasks}"
            assert after_workers == before_workers, f"summary mutated workers: {before_workers} -> {after_workers}"
            assert worker_after.status == before_worker_status, f"summary mutated worker status: {before_worker_status} -> {worker_after.status}"
        finally:
            db.close()


def eval_policy_hook_summary_compatibility():
    """summarize_runtime_policy_hook_evaluations is registered and compatible with other tools."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db = NoraDB(Path(tmpdir) / "test.db")
        try:
            registry = build_default_registry(workspace_root=Path(tmpdir), db=db, confirm_action=lambda _: True)
            ...
            # Tool is registered
            perms_text = registry.call("list_tool_permissions")
            assert "summarize_runtime_policy_hook_evaluations" in perms_text
            ...
            # list_runtime_policy_hook_evaluations still works
            list_result = json.loads(registry.call("list_runtime_policy_hook_evaluations"))
            assert list_result["count"] >= 1

            # summarize_runtime_policy_hook_evaluations works after recording
            summary_result = json.loads(registry.call("summarize_runtime_policy_hook_evaluations"))
            assert summary_result["total"] >= 1

            # Durable task tools still work
            new_task = ts.create_task(goal="compat test summary", steps=[{"text": "step1"}])
            assert ts.get_task(new_task.task_id) is not None
