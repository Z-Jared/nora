...
class WorkspaceMergeFinalizeTests(unittest.TestCase):
    SECRET_SENTINEL = "finalize_secret_789xyz"
    ...
    def _register_and_assign(self, worker_id="w1", goal="task one", status="running"):
        self.registry.call("register_worker", worker_id=worker_id)
        task = json.loads(self.registry.call("create_durable_task", goal=goal, steps="step one"))
        task_id = task["task_id"]
        self.registry.call("assign_durable_task", task_id=task_id, worker_id=worker_id)
        self.registry.call("update_worker_status", worker_id=worker_id, status=status, current_task_id=task_id)
        if status == "running":
    ...
    def test_finalize_after_apply(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])

        result = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w1", task_id=task_id,
        ))
        self.assertTrue(result["finalized"])
        self.assertEqual(result["task_status_before"], "running")
        self.assertEqual(result["task_status_after"], "completed")
        self.assertEqual(result["worker_status_before"], "running")
        self.assertEqual(result["worker_status_after"], "idle")
        self.assertTrue(result["workspace_released"])
        self.assertEqual(result["lease_id"], lease["lease_id"])

    def test_finalize_task_marked_completed(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)

        task = json.loads(self.registry.call("get_durable_task", task_id=task_id))
        self.assertEqual(task["status"], "completed")

    def test_finalize_worker_marked_idle(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)
    ...
    def test_finalize_lease_released(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)
        ...
        lease_info = json.loads(self.registry.call("get_worker_workspace", worker_id="w1", task_id=task_id))
        self.assertIn("error", lease_info)

    # --- no apply event ---

    def test_rejects_without_apply_event(self):
        task_id = self._register_and_assign()
        self._prepare_workspace("w1", task_id)
        ...
        self.assertEqual(result["reason"], "no_successful_apply")

    # --- release_workspace=false ---

    def test_release_workspace_false_keeps_lease(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])

        result = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w1", task_id=task_id, release_workspace=False,
        ))
        self.assertTrue(result["finalized"])
        self.assertFalse(result["workspace_released"])

        # Lease still exists in the store (worker is now idle so get_worker_workspace rejects)
        stored_lease = self.registry.workspace_lease_store.get_lease_by_worker("w1")
        self.assertIsNotNone(stored_lease)
        self.assertEqual(stored_lease.lease_id, lease["lease_id"])

    def test_no_active_lease_blocks_finalize(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.workspace_lease_store.release_lease(lease["lease_id"])
        ...
        self.assertFalse(result["finalized"])
        self.assertEqual(result["reason"], "workspace_lease_invalid")
        task = json.loads(self.registry.call("get_durable_task", task_id=task_id))
        worker = json.loads(self.registry.call("get_worker", worker_id="w1"))
    ...
    def test_apply_event_must_match_active_lease(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.workspace_lease_store.create_lease("w_other", "dtask_other", str(self.root / "other_workspace"))
        self.registry.workspace_lease_store.release_lease(lease["lease_id"])
        replacement_path = str(Path(lease["workspace_path"]).with_name("replacement_workspace"))
        replacement = self.registry.workspace_lease_store.create_lease("w1", task_id, replacement_path)
        ...
        self.assertFalse(result["finalized"])
        self.assertEqual(result["reason"], "no_successful_apply")
        self.assertEqual(result["lease_id"], replacement.lease_id)
        stored_lease = self.registry.workspace_lease_store.get_lease_by_worker("w1")
        self.assertIsNotNone(stored_lease)
        self.assertEqual(stored_lease.lease_id, replacement.lease_id)
        task = json.loads(self.registry.call("get_durable_task", task_id=task_id))
        self.assertEqual(task["status"], "running")

    def test_apply_event_must_not_precede_active_lease(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.workspace_lease_store.release_lease(lease["lease_id"])
        replacement = self._prepare_workspace("w1", task_id)
        self.assertEqual(replacement["lease_id"], lease["lease_id"])
    ...
    def test_invalid_release_workspace_returns_error(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
    ...
    def test_repeated_finalize_returns_already_finalized(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])

        first = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w1", task_id=task_id,
        ))
        self.assertTrue(first["finalized"])

        second = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w1", task_id=task_id,
        ))
        self.assertFalse(second["finalized"])
        self.assertEqual(second["reason"], "already_finalized")
        self.assertEqual(second["task_status_before"], "completed")
        self.assertEqual(second["task_status_after"], "completed")
        self.assertFalse(second["workspace_released"])

    # --- validation errors ---

    def test_unknown_worker_returns_error(self):
        result = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="nope", task_id="dtask_x",
        ))
        self.assertIn("error", result)

    def test_empty_worker_id_returns_error(self):
        result = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="", task_id="dtask_x",
        ))
        self.assertIn("error", result)

    def test_empty_task_id_returns_error(self):
        result = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w1", task_id="",
        ))
        self.assertIn("error", result)
    ...
    def test_task_not_assigned_to_worker_returns_error(self):
        task_id = self._register_and_assign(worker_id="w1")
        self._prepare_workspace("w1", task_id)
        self.registry.call("register_worker", worker_id="w2")

        result = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w2", task_id=task_id,
        ))
        self.assertIn("error", result)
    ...
    def test_task_not_running_returns_error(self):
        ...
        task_id = task["task_id"]
        self.registry.call("assign_durable_task", task_id=task_id, worker_id="w1")
        self.registry.call("update_worker_status", worker_id="w1", status="assigned", current_task_id=task_id)
        self._prepare_workspace("w1", task_id)
        ...
        self.assertFalse(result["finalized"])
        self.assertEqual(result["reason"], "task_not_running")
    ...
    def test_no_goal_leak_in_already_finalized(self):
        task_id = self._register_and_assign(goal=self.SECRET_SENTINEL)
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)
    ...
    def test_no_goal_leak_in_no_apply(self):
        task_id = self._register_and_assign(goal=self.SECRET_SENTINEL)
        self._prepare_workspace("w1", task_id)
    ...
    def test_finalize_event_payload_safe(self):
        task_id = self._register_and_assign(goal=self.SECRET_SENTINEL)
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [
            ("a.py", self.SECRET_SENTINEL),
        ])

        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)

        events = self.registry.durable_event_store.list_events(
            task_id=task_id, worker_id="w1", max_results=20,
        )
        finalized = [
            e for e in events
            if e.event_type == TASK_STATUS_CHANGED
            and (e.payload or {}).get("operation") == "workspace_merge_finalize"
        ]
        released = [
            e for e in events
            if e.event_type == WORKSPACE_RELEASED
            and (e.payload or {}).get("operation") == "workspace_merge_finalize_release"
        ]
        self.assertEqual(len(finalized), 1)
        self.assertEqual(len(released), 1)
        payload = finalized[0].payload
        self.assertEqual(payload["lease_id"], lease["lease_id"])
        serialized = json.dumps([e.to_dict() for e in finalized + released], ensure_ascii=False)
        self.assertNotIn(self.SECRET_SENTINEL, serialized)
        self.assertNotIn("step one", serialized)
    ...
    def test_does_not_mutate_workspace_contents(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
    ...
    def test_existing_tools_still_work_after_finalize(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)
        ...
        # Task tools still work
        task = json.loads(self.registry.call("get_durable_task", task_id=task_id))
        self.assertEqual(task["status"], "completed")

        # Register new worker still works
        self.registry.call("register_worker", worker_id="w2")
        w2 = json.loads(self.registry.call("get_worker", worker_id="w2"))
        self.assertEqual(w2["worker_id"], "w2")


class WorkspaceMergeCloseoutCandidateTests(unittest.TestCase):
    """Tests for list_worker_workspace_merge_closeout_candidates (TASK-081)."""

    SECRET_SENTINEL = "CANDIDATE_SECRET_456"
    ...
    def test_ready_candidate_after_apply(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])

        result = json.loads(self.registry.call("list_worker_workspace_merge_closeout_candidates"))
        self.assertEqual(result["count"], 1)
        c = result["candidates"][0]
        self.assertTrue(c["ready"])
        self.assertEqual(c["reason"], "ready_to_finalize")
        self.assertEqual(c["worker_id"], "w1")
        self.assertEqual(c["task_id"], task_id)
        self.assertEqual(c["lease_id"], lease["lease_id"])
        self.assertEqual(c["task_status"], "running")
        self.assertNotEqual(c["latest_apply_event_id"], "")
        self.assertNotEqual(c["latest_apply_created_at"], "")

    # --- no candidate before apply ---

    def test_not_ready_before_apply(self):
        task_id = self._register_and_assign()
        self._prepare_workspace("w1", task_id)
        ...
        self.assertEqual(c["reason"], "no_successful_apply")

    # --- no worker with task ---

    def test_no_candidates_when_no_workers(self):
        result = json.loads(self.registry.call("list_worker_workspace_merge_closeout_candidates"))
        self.assertEqual(result["count"], 0)
    ...
    def test_already_finalized(self):
        ...
        # After finalization, worker is idle with no current_task_id, so no candidates via all-workers scan
        result = json.loads(self.registry.call("list_worker_workspace_merge_closeout_candidates"))
        self.assertEqual(result["count"], 0)

    def test_already_finalized_by_task_filter(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("finalize_worker_workspace_merge", worker_id="w1", task_id=task_id)

        # Filtering by task_id still finds the completed task
        result = json.loads(self.registry.call(
            "list_worker_workspace_merge_closeout_candidates", task_id=task_id,
        ))
        self.assertEqual(result["count"], 1)
        c = result["candidates"][0]
        self.assertFalse(c["ready"])
        self.assertEqual(c["reason"], "already_finalized")

    # --- offline worker ---

    def test_offline_worker_not_ready(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        ...
        c = result["candidates"][0]
        self.assertFalse(c["ready"])
        self.assertEqual(c["reason"], "worker_unavailable")

    # --- idle worker ---

    def test_idle_worker_not_ready(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.call("update_worker_status", worker_id="w1", status="idle", current_task_id="")
    ...
    def test_task_not_running(self):
        self.registry.call("register_worker", worker_id="w1")
        task = json.loads(self.registry.call("create_durable_task", goal="g", steps="s"))
        task_id = task["task_id"]
        self.registry.call("assign_durable_task", task_id=task_id, worker_id="w1")
        ...
        c = result["candidates"][0]
        self.assertFalse(c["ready"])
        self.assertEqual(c["reason"], "task_not_running")

    # --- no lease ---

    def test_no_lease_not_ready(self):
        task_id = self._register_and_assign()
        # No prepare_workspace call

        result = json.loads(self.registry.call("list_worker_workspace_merge_closeout_candidates"))
        self.assertEqual(result["count"], 1)
        c = result["candidates"][0]
        self.assertFalse(c["ready"])
        self.assertEqual(c["reason"], "workspace_lease_invalid")

    # --- stale apply from old lease ---

    def test_stale_apply_from_old_lease_not_ready(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        self.registry.workspace_lease_store.release_lease(lease["lease_id"])
    ...
    def test_filter_by_worker_id(self):
        ...
        result = json.loads(self.registry.call(
            "list_worker_workspace_merge_closeout_candidates", worker_id="w1",
        ))
        self.assertEqual(result["count"], 1)
        self.assertEqual(result["candidates"][0]["worker_id"], "w1")
    ...
    def test_filter_by_worker_and_task(self):
        task_id = self._register_and_assign(worker_id="w1")
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])

        result = json.loads(self.registry.call(
            "list_worker_workspace_merge_closeout_candidates", worker_id="w1", task_id=task_id,
        ))
        self.assertEqual(result["count"], 1)
        self.assertTrue(result["candidates"][0]["ready"])

    def test_limit_bounds(self):
        for i in range(5):
            wid = f"w{i}"
            tid = self._register_and_assign(worker_id=wid, goal=f"task {i}")
            lease = self._prepare_workspace(wid, tid)
            for rel, content in [("a.py", "a\n")]:
                p = Path(lease["workspace_path"]) / rel
                p.parent.mkdir(parents=True, exist_ok=True)
                p.write_text(content, encoding="utf-8")
            self.registry.call(
                "record_worker_workspace_review_gate",
                worker_id=wid, task_id=tid, decision="approved",
            )
            self.registry.call("apply_reviewed_worker_workspace_merge", worker_id=wid, task_id=tid)

        result = json.loads(self.registry.call(
            "list_worker_workspace_merge_closeout_candidates", limit=3,
        ))
        self.assertEqual(result["count"], 3)
    ...
    def test_query_is_read_only(self):
        ...
        task_before = json.loads(self.registry.call("get_durable_task", task_id=task_id))
        worker_before = json.loads(self.registry.call("get_worker", worker_id="w1"))
        lease_store_before = self.registry.workspace_lease_store.get_lease_by_worker("w1")

        self.registry.call("list_worker_workspace_merge_closeout_candidates")

        task_after = json.loads(self.registry.call("get_durable_task", task_id=task_id))
        worker_after = json.loads(self.registry.call("get_worker", worker_id="w1"))
        lease_store_after = self.registry.workspace_lease_store.get_lease_by_worker("w1")

        self.assertEqual(task_before["status"], task_after["status"])
        self.assertEqual(worker_before["status"], worker_after["status"])
        self.assertEqual(lease_store_before.lease_id, lease_store_after.lease_id)
    ...
    def test_no_goal_leak_not_ready(self):
        task_id = self._register_and_assign(goal=self.SECRET_SENTINEL)
        self._prepare_workspace("w1", task_id)
    ...
    def test_existing_tools_still_work_after_query(self):
        task_id = self._register_and_assign()
        lease = self._prepare_workspace("w1", task_id)
        self._record_gate_and_apply(task_id, lease["workspace_path"], [("a.py", "a\n")])
        ...
        fin = json.loads(self.registry.call(
            "finalize_worker_workspace_merge", worker_id="w1", task_id=task_id,
        ))
...
class WorkerLifecycleSchedulerLoopTests(unittest.TestCase):
    """Tests for run_worker_lifecycle_scheduler_loop (TASK-091)."""

    SECRET_SENTINEL = "SCHEDULER_LOOP_SECRET_XYZ"
    ...
    def test_dry_run_loop_returns_bounded_ticks_no_mutation(self):
        self._setup_ready_worker("w1")
        result = self.registry.call("run_worker_lifecycle_scheduler_loop")
        parsed = json.loads(result)

        self.assertEqual(parsed["scheduler"], "worker_lifecycle")
        self.assertTrue(parsed["dry_run"])
        self.assertIn("loop_id", parsed)
        self.assertGreater(parsed["ticks_run"], 0)
        self.assertLessEqual(parsed["ticks_run"], parsed["max_ticks"])
        self.assertIn(parsed["stopped_reason"], ("idle", "max_ticks_reached"))
        self.assertIn("ticks", parsed)
        self.assertIsInstance(parsed["ticks"], list)
        self.assertEqual(len(parsed["ticks"]), parsed["ticks_run"])
        # Verify no mutation
        task = json.loads(self.registry.call("get_durable_task", task_id=json.loads(self.registry.call("list_durable_tasks"))[0]["task_id"]))
        self.assertEqual(task["status"], "running")

    def test_non_dry_run_finalizes_ready_closeouts(self):
        self._setup_ready_worker("w1")
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", dry_run=False, max_ticks=1)
        parsed = json.loads(result)

        self.assertFalse(parsed["dry_run"])
        self.assertGreaterEqual(parsed["executed_count"], 1)

    def test_max_ticks_clamp_low(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=0)
        parsed = json.loads(result)
        self.assertEqual(parsed["max_ticks"], 1)

    def test_max_ticks_clamp_high(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=99)
        parsed = json.loads(result)
        self.assertEqual(parsed["max_ticks"], 10)

    def test_max_ticks_bool_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=True)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_max_ticks_float_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=1.5)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_max_ticks_string_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks="bad")
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_limit_bool_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", limit=True)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_limit_float_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", limit=1.5)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_limit_string_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", limit="bad")
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_bad_dry_run_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", dry_run="yes")
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_bad_release_workspace_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", release_workspace=1)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_bad_stop_when_idle_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", stop_when_idle=1)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_bad_record_event_returns_error(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", record_event=1)
        parsed = json.loads(result)
        self.assertIn("error", parsed)

    def test_stop_when_idle_true_stops_early_empty_state(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", stop_when_idle=True, max_ticks=10)
        parsed = json.loads(result)

        self.assertLessEqual(parsed["ticks_run"], 2)
        self.assertEqual(parsed["stopped_reason"], "idle")

    def test_stop_when_idle_false_runs_all_ticks(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop", stop_when_idle=False, max_ticks=3)
        parsed = json.loads(result)

        self.assertEqual(parsed["ticks_run"], 3)
        self.assertEqual(parsed["stopped_reason"], "max_ticks_reached")

    def test_dispatch_wait_actions_blocked(self):
        self.registry.call("register_worker", worker_id="w1")
        self.registry.call("update_worker_status", worker_id="w1", status="idle")
        self.registry.call("create_durable_task", goal="g", steps="s")

        result = self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=1)
        parsed = json.loads(result)

        tick = parsed["ticks"][0]
        self.assertGreater(tick["blocked_count"], 0)

    def test_loop_event_recorded_when_true(self):
        self._setup_ready_worker("w1")
        self.registry.call("run_worker_lifecycle_scheduler_loop", record_event=True, max_ticks=1)

        events = self.registry.durable_event_store.list_events()
        loop_events = [e for e in events if e.event_type == SCHEDULER_DECISION and e.summary == "scheduler loop"]
        self.assertGreater(len(loop_events), 0)
        event = loop_events[0]
        self.assertEqual(event.source, "scheduler")
        self.assertEqual(event.severity, "info")
        self.assertIn("loop_id", event.payload)
        self.assertEqual(event.payload["scheduler"], "worker_lifecycle")

    def test_no_loop_event_when_false(self):
        self._setup_ready_worker("w1")
        self.registry.call("run_worker_lifecycle_scheduler_loop", record_event=False, max_ticks=1)

        events = self.registry.durable_event_store.list_events()
        loop_events = [e for e in events if e.event_type == SCHEDULER_DECISION and e.summary == "scheduler loop"]
        self.assertEqual(len(loop_events), 0)

    def test_permission_requires_confirmation(self):
        reg = build_default_registry(db=self.db, workspace_root=self.root, confirm_action=lambda _: False)
        result = reg.call("run_worker_lifecycle_scheduler_loop")
        self.assertIn("取消", result)
    ...
    def test_no_steps_leak(self):
        ...
        result = self.registry.call("run_worker_lifecycle_scheduler_loop")
        parsed = json.loads(result)
        for tick in parsed.get("ticks", []):
            self.assertNotIn("step", json.dumps(tick))
    ...
    def test_event_payload_no_goal_leak(self):
        ...
        loop_events = [e for e in events if e.event_type == SCHEDULER_DECISION and e.summary == "scheduler loop"]
        self.assertGreater(len(loop_events), 0)
        self.assertNotIn(self.SECRET_SENTINEL, json.dumps(loop_events[0].payload))

    def test_output_has_required_fields(self):
        result = self.registry.call("run_worker_lifecycle_scheduler_loop")
        parsed = json.loads(result)
        for field in ["scheduler", "loop_id", "dry_run", "max_ticks", "ticks_run",
                       "stopped_reason", "planned_count", "executed_count", "skipped_count",
                       "failed_count", "blocked_count", "ticks", "summary"]:
            self.assertIn(field, parsed)

    def test_compatibility_with_scheduler_tick(self):
        self._setup_ready_worker("w1")
        tick_result = json.loads(self.registry.call("run_worker_lifecycle_scheduler_tick"))
        loop_result = json.loads(self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=1))
        self.assertIn("tick_id", tick_result)
        self.assertIn("loop_id", loop_result)

    def test_compatibility_with_run_once(self):
        self._setup_ready_worker("w1")
        once_result = json.loads(self.registry.call("run_worker_lifecycle_once"))
        loop_result = json.loads(self.registry.call("run_worker_lifecycle_scheduler_loop", max_ticks=1))
        self.assertIn("planned_count", once_result)
        self.assertIn("planned_count", loop_result)

    def test_compatibility_with_planner(self):
        self._setup_ready_worker("w1")
        plan_result = json.loads(self.registry.call("plan_worker_lifecycle_actions"))
        self.assertIn("actions", plan_result)
...
class SummarizeRuntimePolicyHookEvaluationsTests(unittest.TestCase):
    """Tests for summarize_runtime_policy_hook_evaluations (TASK-107)."""

    SECRET_SENTINEL = "SUMMARIZE_POLICY_SECRET_XYZ"
    SHELL_SENTINEL = "rm -rf /important"
    ...
    def _summarize(self, **kwargs):
        return json.loads(self.registry.call("summarize_runtime_policy_hook_evaluations", **kwargs))
    ...
    def test_empty_summary_returns_zeros(self):
        ...
        self.assertEqual(result["decisions"], {"allow": 0, "confirm": 0, "block": 0})
        self.assertEqual(result["hooks"], {})
        self.assertEqual(result["categories"], {})
        self.assertEqual(result["risks"], {})
        self.assertEqual(result["requires_confirmation_count"], 0)
        self.assertEqual(result["blocked_count"], 0)
        self.assertEqual(result["recent_event_ids"], [])
        self.assertEqual(result["policy_versions"], {})
        self.assertNotIn("errors", result)

    def test_summary_counts_decisions(self):
        self._record(hook="pre_tool", risk="read")  # allow
        self._record(hook="pre_tool", risk="write")  # confirm
        self._record(hook="pre_shell", risk="destructive")  # block
        result = self._summarize()
        self.assertEqual(result["total"], 3)
        self.assertEqual(result["decisions"]["allow"], 1)
        self.assertEqual(result["decisions"]["confirm"], 1)
        self.assertEqual(result["decisions"]["block"], 1)
    ...
    def test_summary_counts_hooks(self):
        ...
        self._record(hook="pre_shell", risk="write")
        result = self._summarize()
        self.assertEqual(result["hooks"]["pre_tool"], 2)
        self.assertEqual(result["hooks"]["pre_shell"], 1)

    def test_summary_counts_categories(self):
        self._record(hook="pre_tool", risk="read", category="file")
        self._record(hook="pre_shell", risk="write", category="shell")
        result = self._summarize()
        self.assertEqual(result["categories"]["file"], 1)
        self.assertEqual(result["categories"]["shell"], 1)

    def test_summary_counts_risks(self):
        self._record(hook="pre_tool", risk="read")
        self._record(hook="pre_tool", risk="write")
        result = self._summarize()
        self.assertEqual(result["risks"]["read"], 1)
        self.assertEqual(result["risks"]["write"], 1)

    def test_summary_policy_versions(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize()
        self.assertIn("v1", result["policy_versions"])
        self.assertEqual(result["policy_versions"]["v1"], 1)

    def test_summary_recent_event_ids(self):
        r1 = self._record(hook="pre_tool", risk="read")
        r2 = self._record(hook="pre_shell", risk="write")
        result = self._summarize()
        self.assertEqual(len(result["recent_event_ids"]), 2)
        # newest first
        self.assertEqual(result["recent_event_ids"][0], r2["event_id"])
        self.assertEqual(result["recent_event_ids"][1], r1["event_id"])
    ...
    def test_hook_filter(self):
        self._record(hook="pre_tool", risk="read")
        self._record(hook="pre_shell", risk="write")
        result = self._summarize(hook="pre_tool")
        self.assertEqual(result["total"], 1)
        self.assertEqual(result["hooks"], {"pre_tool": 1})

    def test_decision_filter(self):
        self._record(hook="pre_tool", risk="read")  # allow
        self._record(hook="pre_tool", risk="write")  # confirm
        result = self._summarize(decision="allow")
        self.assertEqual(result["total"], 1)
        self.assertEqual(result["decisions"]["allow"], 1)
        self.assertEqual(result["decisions"]["confirm"], 0)

    def test_category_filter(self):
        self._record(hook="pre_tool", risk="read", category="file")
        self._record(hook="pre_shell", risk="write", category="shell")
        result = self._summarize(category="file")
        self.assertEqual(result["total"], 1)
        self.assertEqual(result["categories"], {"file": 1})
    ...
    def test_risk_filter(self):
        self._record(hook="pre_tool", risk="read")
        self._record(hook="pre_tool", risk="write")
        result = self._summarize(risk="read")
        self.assertEqual(result["total"], 1)
        self.assertEqual(result["risks"], {"read": 1})
    ...
    def test_task_id_filter(self):
        self._record(hook="pre_tool", risk="read", task_id="t1")
        self._record(hook="pre_tool", risk="read", task_id="t2")
        result = self._summarize(task_id="t1")
        self.assertEqual(result["total"], 1)

    def test_worker_id_filter(self):
        self._record(hook="pre_tool", risk="read", worker_id="w1")
        self._record(hook="pre_tool", risk="read", worker_id="w2")
        result = self._summarize(worker_id="w1")
        self.assertEqual(result["total"], 1)

    def test_session_id_filter(self):
        self._record(hook="pre_tool", risk="read", session_id="s1")
        self._record(hook="pre_tool", risk="read", session_id="s2")
        result = self._summarize(session_id="s1")
        self.assertEqual(result["total"], 1)

    # --- Invalid/unsafe filters return empty with errors ---

    def test_invalid_hook_filter_returns_empty(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(hook="INVALID_HOOK_XYZ")
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_hook", result["errors"])
        self.assertNotIn("INVALID_HOOK_XYZ", json.dumps(result))
    ...
    def test_unsafe_hook_filter_returns_empty(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(hook=self.SECRET_SENTINEL)
        self.assertNotIn(self.SECRET_SENTINEL, json.dumps(result))
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
    ...
    def test_invalid_decision_filter_returns_empty(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(decision="INVALID_DECISION")
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_decision", result["errors"])
    ...
    def test_invalid_category_filter_returns_empty(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(category="INVALID_CATEGORY")
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_category", result["errors"])

    def test_invalid_risk_filter_returns_empty(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(risk="INVALID_RISK")
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_risk", result["errors"])

    def test_unsafe_task_id_returns_empty(self):
        self._record(hook="pre_tool", risk="read", task_id="t1")
        result = self._summarize(task_id=self.SECRET_SENTINEL)
        self.assertNotIn(self.SECRET_SENTINEL, json.dumps(result))
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_task_id", result["errors"])

    def test_unsafe_worker_id_returns_empty(self):
        self._record(hook="pre_tool", risk="read", worker_id="w1")
        result = self._summarize(worker_id=self.SECRET_SENTINEL)
        self.assertNotIn(self.SECRET_SENTINEL, json.dumps(result))
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_worker_id", result["errors"])

    def test_unsafe_session_id_returns_empty(self):
        self._record(hook="pre_tool", risk="read", session_id="s1")
        result = self._summarize(session_id=self.SECRET_SENTINEL)
        self.assertNotIn(self.SECRET_SENTINEL, json.dumps(result))
        self.assertEqual(result["total"], 0)
        self.assertIn("errors", result)
        self.assertIn("invalid_session_id", result["errors"])
    ...
    def test_limit_bounded(self):
        ...
        self.assertEqual(result["total"], 3)
        self.assertEqual(result["filters"]["limit"], 3)
        self.assertEqual(len(result["recent_event_ids"]), 3)

    def test_limit_clamped_to_max(self):
        for i in range(3):
            self._record(hook="pre_tool", risk="read")
        result = self._summarize(limit=999)
        self.assertEqual(result["filters"]["limit"], 100)

    def test_limit_invalid_defaults(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(limit="invalid")
        self.assertEqual(result["filters"]["limit"], 20)

    def test_limit_zero_clamps_to_one(self):
        self._record(hook="pre_tool", risk="read")
        result = self._summarize(limit=0)
        self.assertEqual(result["filters"]["limit"], 1)
        self.assertEqual(result["total"], 1)
    ...
    def test_summarize_does_not_create_events(self):
        self._record(hook="pre_tool", risk="read")
        before = len(self._get_policy_events())
        self._summarize()
        after = len(self._get_policy_events())
    ...
    def test_summarize_does_not_mutate_tasks(self):
        self._record(hook="pre_tool", risk="read")
        tasks_before = json.loads(self.registry.call("list_durable_tasks"))
        self._summarize()
        tasks_after = json.loads(self.registry.call("list_durable_tasks"))
    ...
    def test_summarize_does_not_mutate_workers(self):
        self.registry.call("register_worker", worker_id="wf")
        workers_before = json.loads(self.registry.call("list_workers"))
        self._summarize()
        workers_after = json.loads(self.registry.call("list_workers"))
    ...
    def test_list_tool_permissions_includes_new_tool(self):
        result = self.registry.call("list_tool_permissions")
        self.assertIn("summarize_runtime_policy_hook_evaluations", result)
        # Should be read-only
        self.assertIn("read", result.split("summarize_runtime_policy_hook_evaluations")[-1].split("\n")[0])
